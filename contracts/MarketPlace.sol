...
contract MarketPlace is ReentrancyGuard{
  ...
  function createMarketAuction(address nftContract,uint256 tokenId,uint256 floorPrice,uint auctionDays) external payable nonReentrant{
  ...
  function createAuctionSale(address nftContract,uint itemId) external payable nonReentrant{
        MarketItem storage currentItem = idToMarketItem[itemId];
        AuctionInfo storage currentInfo = auctionData[itemId];
        require(!currentItem.sold,"Item has already been sold");
        require(currentItem.isAuction,"Item is not for auction");
